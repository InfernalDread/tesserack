...
import {
  ...
  is_void,
  noop
} from "./chunk-3DN4NXDP.js";
import {
  async_mode_flag
} from "./chunk-XVE2VFRE.js";
import {
  true_default
...
function get_render_context() {
  const store = context ?? (als == null ? void 0 : als.getStore());
  if (!store) {
...
function init_render_context() {
  als_import ?? (als_import = import("./node_async_hooks-56V2J4QD.js").then((hooks) => {
...
function in_webcontainer() {
  var _a, _b;
  return !!((_b = (_a = globalThis.process) == null ? void 0 : _a.versions) == null ? void 0 : _b.webcontainer);
}
...
async function sha256(data) {
  ...
  crypto ?? (crypto = ((_b = (_a = globalThis.crypto) == null ? void 0 : _a.subtle) == null ? void 0 : _b.digest) ? globalThis.crypto : (
    ...
    (await import("./node_crypto-RWHF4AVO.js")).webcrypto
  ));
  const hash_buffer = await crypto.subtle.digest("SHA-256", text_encoder.encode(data));
  return base64_encode(hash_buffer);
}
function base64_encode(bytes) {
  if (!true_default && globalThis.Buffer) {
  ...
  function stringify3(thing) {
    ...
    switch (type) {
      ...
      default:
        ...
        const obj = keys2.map((key) => `${safe_key(key)}:${stringify3(thing[key])}`).join(",");
        const proto = Object.getPrototypeOf(thing);
        if (proto === null) {
      ...
      switch (type) {
        ...
        default:
          ...
          Object.keys(thing).forEach((key) => {
            statements.push(
              `${name}${safe_prop(key)}=${stringify3(thing[key])}`
            );
          });
      }
    });
...
var _out, _on_destroy, _is_component_body, _parent, _Renderer_instances, collect_on_destroy_fn, traverse_components_fn, collect_ondestroy_fn, _Renderer_static, render_fn, render_async_fn, collect_content_fn, collect_content_async_fn, collect_hydratables_fn, open_render_fn, close_render_fn, hydratable_block_fn;
var _Renderer = class _Renderer {
  ...
  option(attrs, body, css_hash, classes, styles, flags, is_rich) {
    ...
    const close = (renderer, value, { head: head2, body: body2 }) => {
      ...
      __privateGet(renderer, _out).push(`>${body2}${is_rich ? "<!>" : ""}</option>`);
      if (head2) {
      ...
      then: {
        value: (
          ...
          (onfulfilled, onrejected) => {
            ...
            async ?? (async = init_render_context().then(
              () => with_render_context(() => {
                var _a2;
                return __privateMethod(_a2 = _Renderer, _Renderer_static, render_async_fn).call(_a2, component, options);
              })
            ));
...
//# sourceMappingURL=chunk-E4WJU3GM.js.map
